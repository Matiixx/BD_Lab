...
int main(int argc, char **argv)
{
    ...
    strcpy(wszInput, "SELECT lname, fname, i.description, SUM(ol.quantity) FROM lab11.customer JOIN lab11.orderinfo USING(customer_id) JOIN lab11.orderline ol USING(orderinfo_id) JOIN lab11.item i USING(item_id) WHERE lname = '"); //WHERE customer_id = ?
    strcat(wszInput, argv[1]);
    strcat(wszInput, "' GROUP BY lname, fname, i.description;");
